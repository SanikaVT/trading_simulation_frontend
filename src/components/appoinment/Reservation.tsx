...
export default function Reservation() {
  ...
  const advisor = data.map((advisor) =>
    advisor.firstName.concat("-").concat(advisor.lastName)
  );

  // create a appointment
  const handleClick = (e: any) => {
    axios
      .get("/api/advisor/" + advisorName)
      .then((result) => {
        const data = {
          fullName: advisorName,
          address: address,
          date: value,
          email: result.data.advisor.email,
          firstName: result.data.advisor.firstName,
          lastName: result.data.advisor.lastName,
          age: result.data.advisor.age,
  ...
  if (localStorage.getItem("userID")) {
        ...
        <Box sx={{ flexGrow: 1 }}>
          ...
          >
            <Grid item xs={2} sm={2} md={4} sx={{ gap: 1 }}>
              <Autocomplete
                onChange={(event, value) =>
                  value ? setAdvisorName(value) : null
                }
                disablePortal
                id="advisor"
                options={advisor}
                sx={{ width: 300, gap: 2 }}
                renderInput={(params) => (
                  <TextField {...params} label="Advisor" />
                )}
              />
            </Grid>
            <Grid item xs={2} md={4} sm={2}>
              <Autocomplete
                onChange={(event, value) =>
                  value ? setAddress(value.label) : null
                }
                disablePortal
                id="city"
                options={cities}
    ...
    return <>your need to log in to book an appointment</>;
  }
}
